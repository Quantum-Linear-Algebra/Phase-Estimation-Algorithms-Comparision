...
if computation_type == 'S' or computation_type == 'H':
    Dt = parameters['Dt']
    filename = "0-Data/Transpiled_Circuits/"+make_filename(computation_type, num_timesteps, parameters)+"_Re.qpy"
    if empty(filename):
    ...
    filename = "0-Data/Transpiled_Circuits/"+make_filename(computation_type, num_timesteps, parameters)+"_Im.qpy"
    if empty(filename):
...
if computation_type == 'S' or computation_type == 'H':
    ...
    if computation_type == 'H':
        ...
        for job in jobs:
            job_ids +=job.job_id()
        with open("Jobs/"+job_ids+".pkl", "wb") as file:
...
elif computation_type == 'S' or computation_type == 'H' or computation_type == 'J':
...
with open("0-Data/Expectation_Values/"+make_filename(computation_type, num_timesteps, parameters, add_shots=True)+".pkl", "wb") as file:
    pickle.dump(exp_vals, file)

print("Saved expectation values into file.", "(0-Data/Expectation_Values/"+make_filename(computation_type, num_timesteps, parameters, add_shots=True)+".pkl)")